...
<#
.SYNOPSIS
    Package modules using a given folder after installing to a PowerShell module path ($env:PSModulePath).
...
<#
.SYNOPSIS
    Deploys each DSC Resource module to the $env:PSModulePath and creates a zip package for one in the destination provided
.PARAMETER Source
